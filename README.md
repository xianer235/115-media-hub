...
### 直接推送到接收夹：`POST /webhook/quick-import`

不想让磁力直接落在某个监控目录时，用这个独立入口把它投进**接收夹**：离线下载完成后会自动跑一次快捷导入（识别 → 整理 → 按类型搬进电影/电视剧监控目录）。

- 与 `/webhook/{任务名}` 完全独立，不需要监控任务、也不影响原有按任务推送的行为；油猴脚本里把它当作一条普通任务，请求地址填 `http://IP:端口/webhook/quick-import` 即可（HTTPS 反代同理填 `https://域名/webhook/quick-import`）
- 需要先在「文件夹监控」页启用**接收夹快捷导入**并选好接收文件夹、标注电影/电视剧目标，否则返回 400 并写明原因
- `savepath` 可以留空（默认用配置好的接收夹），也可以填接收夹本身或它的子目录；填成接收夹以外的目录会直接报错
- 请求体只接受磁力链接（`magnet` / `link_url` / `url` / `link` 任一字段），鉴权方式与 `/webhook/{任务名}` 完全一致（`X-Webhook-Token` 或 `X-Webhook-Ts`/`X-Webhook-Nonce`/`X-Webhook-Sign` 签名）
...
- 当前版本：`0.11.8`
- 新增接收夹专属 webhook：`POST /webhook/quick-import` —— 在「文件夹监控」页的「接收夹快捷导入」卡片点「设置 / 推送地址」，二级弹窗里会显示当前域名下的该地址并提供「复制地址」按钮（参数配置页也有说明），把它填进油猴脚本新增任务的"请求地址"即可，磁力会**直接投进接收夹**，离线下载完成后自动识别整理并按类型搬进电影/电视剧监控目录；与原有的 `/webhook/{任务名}` 完全独立，原有按任务推送不受影响（详见下方「Webhook 说明」）。
- 界面收敛：「接收夹快捷导入」卡片现在只显示两行摘要（接收夹 + 分发目标 / 最近一次结果）、启用开关和「立即整理并分发」「设置 / 推送地址」两个按钮，接收文件夹选择、分发目标、推送地址与执行明细都收进二级弹窗，页面不再一条条摊开。
