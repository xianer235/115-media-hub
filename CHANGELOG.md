...
## [2.3.4] - 2026-04-05
- 版本号更新为 `2.3.4`，用于本次补发版。
- 保留订阅候选导入超时后自动跳过与默认 2 秒间隔策略。
- 版本信息构建时间调整为 `2026-04-05 17:54`（UTC+8）。

## [2.3.3] - 2026-04-05
- 订阅候选资源导入新增硬超时保护：单候选超时后自动标记失败并继续尝试下一个候选，避免整轮订阅执行被卡住。
- 候选资源超时时会主动取消对应导入任务，减少后台残留运行态任务影响后续调度。
- 订阅候选尝试间隔默认调整为 `2` 秒，降低连续提交压力。
- 版本信息更新至 `2.3.3`，构建时间调整为 `2026-04-05 17:51`（UTC+8）。
