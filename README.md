...
- 当前版本：`0.11.8`
- 接收夹并入任务列表：接收夹现在是「文件夹监控」页里的一个**接收夹任务**（`task_type=inbox`），和扫描任务并列显示、共用同一个任务编辑弹窗与 webhook 口径；任务里直接选“电影分发到哪个扫描任务 / 电视剧分发到哪个扫描任务”，卡片与弹窗显示最近 24 小时接收数量和最近一次整理结果。旧的全局开关 + 独立入口 `POST /webhook/quick-import` 已删除，油猴脚本统一填 `POST /webhook/{任务名}`，保存路径统一按 115 根目录相对路径填（照抄面板的 `/115/接收` 也会自动剥离前缀）。
