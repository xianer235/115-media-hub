...
| 刮削管理 | 网盘文件浏览、TMDB 识别绑定、批量重命名预览与执行，支持任务中心统一管理；可选接入 OpenAI 兼容大模型（DeepSeek/Qwen/OpenAI/Ollama 等），对未自动匹配的条目给出「AI 建议」候选，人工确认后才执行 |
...
- 网盘：`115` / `Quark（夸克）` / `天翼云盘` / `123云盘` / `阿里云盘`，通过 Cookie / 访问凭证驱动（115 支持内置扫码登录，阿里云盘支持内置官方 OAuth 扫码授权）；115 支持分享转存、磁力离线与 `.strm` 生成，其余网盘以分享/链接入库为主
...
### 直接推送到接收夹：用内置的「接收夹」任务

不想让磁力直接落在某个监控目录时，用「文件夹监控」页里内置的**接收夹任务**（默认名 `接收`，可改名；全新安装就已经有一个，不能新建第二个也不能删除）。油猴脚本照样把请求地址填 `http://IP:端口/webhook/{任务名}`。离线下载完成后会自动跑一次识别 → 整理 → 按类型搬进电影/电视剧监控目录。

- 接收夹和扫描任务共用同一套任务模型：同一个任务列表、同一个启用开关、同一种 webhook 地址格式，只是任务类型不同；它默认启用，但 webhook 默认关闭——需要先在「参数配置 → 后台安全管理」设置 Webhook 签名密钥才能开启（避免在没设密钥时暴露免鉴权入口）
- 任务类型是不可更改的：内置接收夹不会被改成扫描任务，普通监控任务也不会被改成接收夹，「新增任务」一律创建普通监控任务；接收夹只调整它的接收目录、分发目标与开关（改名可以，但会同时改变它的 webhook 地址）
- 任务里要选好接收文件夹，并指定“电影分发到哪个扫描任务 / 电视剧分发到哪个扫描任务”，否则返回 400 并写明原因
- 「启用」关掉后，定时、webhook、导入完成、刮削变更等自动触发都会停，只保留手动运行；「定时执行」对接收夹同样生效，会周期整理并分发接收夹里的内容
- `savepath` 可以留空（默认落到接收夹），也可以填接收夹本身或它的子目录；填成接收夹以外的目录会直接报错
- `savepath` 统一按 **115 根目录下的相对路径**填写（例如 `接收`、`接收/子目录`）。面板里显示成 `/115/接收` 时照抄进脚本也能识别（会自动剥掉挂载前缀），但推荐只写根目录相对路径
- 请求体只接受磁力链接（`magnet` / `link_url` / `url` / `link` 任一字段），鉴权方式与普通任务完全一致（`X-Webhook-Token` 或 `X-Webhook-Ts`/`X-Webhook-Nonce`/`X-Webhook-Sign` 签名）
- 同一条磁力重复推到接收夹会返回 409（与普通导入一致的查重口径）
...
- 点磁力/torrent 链接旁的「115」后，任务选择弹窗里除推送任务外还有同级的「复制磁力链接」入口；torrent 链接会先自动转成磁力再复制，复制后弹窗保持打开，可继续选择推送任务
- 在离线任务提交后顺手触发服务端刷新
- 复制 115 / Quark 分享链接时保留快捷操作，不强制提交到后台
- 在「任务管理器」里可配置「不生效的网页（排除域名）」，匹配域名下脚本不注入按钮（每行一个域名，支持带协议/端口/路径/通配符，裸域名同时匹配其子域名）
...
3. 点击任意磁力/torrent 链接旁的“115”按钮：未配置任务时直接打开任务管理器，已配置时弹出任务选择器（底部有“任务管理”入口，列表里另有“复制磁力链接”可直接复制磁力、推送与否都不影响）；配置保存在脚本全局存储中，跨网站生效。
...
- 当前版本：`0.11.12`
- 文件夹监控「运行记录」复查修复与补齐：增量变更同步的详情现在会列出它**实际执行的网盘重命名 / 移动**（原路径 → 新路径），升级后首次启动会把历史上“完成时间落在唯一一条变更同步运行区间内”的旧事件自动补齐归属；失败与「部分完成」不再只显示一个状态，而是在「问题」里写明未完成内容与下一步。运行记录页在「任务筛选」选中具体任务后出现**「清空该任务记录」**（先预览条数再确认，运行中 / 排队中 / 等待后续同步的记录保留），筛选区新增**「重置筛选」**；历史文本日志弹窗补回**「加载更早的日志」**，并可单独「清空日志」（会连 `monitor.log.1`、`monitor.log.2` 轮转备份一起清）。顺带修掉三处：运行记录列表不再随每次状态推送整块重绘、运行记录查询补索引并合并子查询（实测 2.2 万条记录 137ms → 1.2ms）、运行详情弹窗的「刷新 / 关闭」固定贴右不再随窗口宽度漂移；结论文案改成中文说明句（如“检查完成：新增或更新 N 个本地播放文件，清理 M 个。”），`/monitor/runs` 的非法 `limit` / `offset` 不再返回 500。
- 接收夹整理可以中断：整理过程中任务卡片上的按钮会变成琥珀色的「中断」（弹窗里的「立即整理并分发」是同一个开关），点了以后在**下一条目开始前**停住——已整理完的不回滚，没处理的留在接收夹并写明「已中断，未整理」；顺手修掉了"已有整理在跑时手动再点会卡住请求"的问题。
- 接收夹并入任务列表：接收夹现在是「文件夹监控」页里的一个**内置接收夹任务**（`task_type=inbox`）——默认就已经存在、有且只有一个、类型不可修改、也不能删除，只配置接收目录与电影/电视剧分发目标；它和扫描任务并列显示、共用同一个编辑弹窗与 webhook 口径，「新增任务」创建的都是普通监控任务。卡片与弹窗显示最近 24 小时接收数量和最近一次整理结果（30 秒自动刷新），整理日志写进同一份「监控日志」并成为独立任务分段。旧的全局开关 + 独立入口 `POST /webhook/quick-import` 已删除，油猴脚本统一填 `POST /webhook/{任务名}`，保存路径统一按 115 根目录相对路径填（照抄面板的 `/115/接收` 也会自动剥离前缀）。
- 安全与停用语义：内置接收夹的 webhook 默认关闭，只有先在「参数配置 → 后台安全管理」设置签名密钥才能开启（签名密钥不再被 `/get_settings` 明文回传）；任务「停用」现在会让定时、webhook、导入完成、刮削变更等所有自动触发一起停下，只保留手动运行；接收夹任务支持「定时执行」周期整理，任务执行延时也生效。
- 配色修复：日间模式下"深灰底翻成浅底但文字仍是白色"导致部分按钮看不见（新弹窗的「复制地址」「关闭」实测对比度仅 1.09:1），现在翻转底色时同步切成深色文字并覆盖 hover；夜间把新弹窗的小标签提亮一档（3.77:1 → 6.99:1）。
- 整理不再新建重复文件夹：同一部剧的 `片名 (2026)`、`片名 (2026) [tmdbid-123]`、`片名 (2026)(1)` 现在被视为同一个文件夹并复用，快捷导入分发时优先**并进**监控目录里已有的那个文件夹（子目录如 `Season 01` 按同层合并，空壳文件夹自动清掉）；已经在 `片名 (年份) [tmdbid-x]/Season 01/` 里的文件不会再被自动刮削套一层同名文件夹，历史嵌套再次整理会被拉平；接收夹里同剧的多个文件夹也不会再因为改名撞车而留在接收夹。散文件强制归档时改为沿用监控任务的命名设置（写入 TMDB ID、建 Season 子目录），与"选中文件夹"整理保持同一套结构。监控任务页「接收夹快捷导入」区块同步重排为左侧配置 / 右侧最近一次结果的两栏布局，状态面板含日间配色。
- 修复散文件没有被整理：接收夹里直接躺在根目录的单个文件（电影 / 剧集），现在会先整理成 `片名 (年份)/`（剧集为 `剧名 (年份)/Season 01/`）再搬进影库；即使监控任务的文件命名方式选的是「保持原名 / 仅清理广告」，也会建出媒体文件夹，只是文件名保持原样。同时修掉了监控任务对「监控根目录散文件」可能把监控目录整目录改名的隐患，以及同剧集多集只整理第一个文件的问题。
- 接收夹快捷导入：监控任务页顶部配置「接收文件夹」并启用后，推送到这里的文件会按识别结果（TMDB 优先、AI 兜底）自动整理，再按电影/剧集搬进对应监控目录，导入时不用再手选保存位置；识别不准的留在接收夹并写明原因。整理流程与监控目录的自动刮削**共用同一套**（选项取自目标监控任务），搬运带 `scraper-job` 来源标记，搬过去后只生成 STRM、不会被二次整理。监控任务里用「接收夹快捷导入目标」标注电影/电视剧即可；CLI 用 `115 monitor quick-import-status` / `115 monitor quick-import-run`。
- 文件夹监控的底部「运行记录」按任务、来源和状态展示扫描、接收夹整理与 STRM 同步结果。普通局部刷新显示实际目录名，接收夹在识别完成后显示影视名称；详情将网盘操作、STRM 操作和问题诊断分开。失败或部分完成的普通扫描可按原范围重试，尚未派发的单条运行可取消。记录默认长期保留，也可设置按天自动清理或预览后手动清理。
- 油猴脚本（`@version` 2.7.0）：点磁力/torrent 链接旁的「115」后，任务选择弹窗里新增与推送任务同级的「复制磁力链接」入口，torrent 会先自动转成磁力再复制，复制后弹窗保持打开。
...
- 设置页「网盘认证与签到」布局优化：扫码/内置授权作为「推荐」置顶、手动粘贴降为「备用」，115 与阿里云样式统一，修复左对齐与日/夜适配；每张网盘卡新增「复制凭证」一键复制已保存的 Cookie/access_token。
- 批量整理修复：剧集文件夹内多选文件预览不再只显示第一个；手动输入集数后预览立即刷新；文件夹重命名但文件已是标准名时，文件行降级为“随文件夹重命名”被动行（网盘侧仍只执行一次文件夹改名）。
- 设置页新增“批量整理过滤词”（第 9 节）：复合过滤词按任意位置删除、独立过滤词按词边界删除，内置默认词表保留、只追加自定义词，保存即生效。
- 115 官方搜索结果路径修复：结果不再被挂到当前目录下伪造路径，未知真实路径的改名/移动/复制/删除/批量整理需先进入文件夹；进入搜索结果文件夹后路径栏显示真实完整路径（官方 medialist 祖先链），URL 写入真实路径，刷新后路径栏可恢复。
- 标题清洗新增“国语音轨”等排除关键词；重命名框鼠标拖选越出小窗口不再误关。
