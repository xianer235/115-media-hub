...
- 当前版本：`0.11.13`
- 文件夹监控「运行记录」按**工作单元**分组显示：默认列表只列触发记录，它派生出来的「增量变更同步 → 自动补扫」缩进在组内（可折叠、最多内联 10 项），组的最近活动时间从下游上溯到组头，所以“接收夹整理”始终排在它自己派生出来的任务之前，不再夹在中间。**接收夹整理不再假部分完成**：自动补扫会挂回发起它的变更同步运行，补扫结束后整条链路按真实结果定稿——6 部影视都同步成功就是「已完成」，只有真的失败 / 仍留在接收夹 / 事件未解决才「部分完成」，结论写明未完成的是哪一项；升级后首次启动会把历史上“只是等补扫、实际已全部完成”的误判记录重新结算（真实失败不动），自动补扫的运行记录也改标为「系统补扫」。
