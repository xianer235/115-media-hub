...
- 当前版本：`0.11.14`
- 文件夹监控「运行记录」**一条任务一行**：接收夹整理是一条任务，它分发出去触发的每条扫描/自动补扫也各自是一条任务，不做分组/折叠；列表按时间排序时触发记录排在自己派生出来的任务之前（活动时间从下游上溯，同秒也保证顺序），所以“最近接收”不会夹在那 6 条任务中间。**接收夹整理不再假部分完成**：自动补扫会挂回发起它的变更同步运行，补扫结束后整条链路按真实结果定稿——6 部影视都同步成功就是「已完成」，只有真的失败 / 仍留在接收夹 / 事件未解决才「部分完成」，结论写明未完成的是哪一项；升级后首次启动会把历史上“只是等补扫、实际已全部完成”的误判记录重新结算（真实失败不动），自动补扫的运行记录也改标为「系统补扫」。
