...
接收夹是**分类前的中转文件夹**，也是新增的一个**便捷入口**。以前电影、电视剧各自只有一个监控目录，保存或推送时必须先想清楚这次要落到哪个分类，在油猴脚本里还得先切到对应的那条任务；现在多了接收夹，磁力、分享转存、手动保存都可以先统一丢进这里，不用每次挑分类。接收到内容后会先识别它是电影还是电视剧，再按对应分类监控任务（分发目标）的刮削 / 整理预设整理好，并移动进那个分类的监控目录；识别不准的留在接收夹并写明原因。

接收夹**不是强制流程**：原来的用法完全保留——给电影、电视剧各建一个监控任务，推送、转存或手动保存时直接指定那个分类目录，行为和以前一样。接收夹只是让你多一个"不想挑分类就先丢进去"的选择，两者可以同时用。

接收夹是「文件夹监控」页里内置的一条**接收夹任务**（默认名 `接收`，可改名；全新安装就已经有一个，不能新建第二个也不能删除）。油猴脚本照样把请求地址填 `http://IP:端口/webhook/{任务名}`，保存路径填接收夹本身或它的子目录（留空默认落到接收夹）。离线下载完成后会自动跑一次识别 → 整理 → 按类型搬进电影/电视剧监控目录。

- **和普通监控任务共用同一个 Webhook 签名密钥**：全站只有一个 `webhook_secret`（在「参数配置 → 后台安全管理」设置），普通监控任务和接收夹任务都用它校验 `X-Webhook-Token` 或 `X-Webhook-Ts`/`X-Webhook-Nonce`/`X-Webhook-Sign`，接收夹不需要、也不支持单独配密钥；两者唯一的区别是**收到请求之后的处理流程不同**——普通监控任务刷新 STRM 并生成播放文件，接收夹任务先识别、整理再按类型分发
- 接收夹和扫描任务共用同一套任务模型：同一个任务列表、同一个启用开关、同一种 webhook 地址格式，只是任务类型不同；它默认启用，但 webhook 默认关闭——需要先设置上面那个共用的签名密钥才能开启（避免在没设密钥时暴露免鉴权入口）
...
- 脚本"保存路径 savepath"是磁力离线下载到 115 的目标目录；普通监控任务要求它落在该任务的扫描路径内（拼上 115 挂载前缀后匹配），导入成功后才会自动刷新并生成 `.strm`
- 想省掉"每次保存先选分类"时，用内置接收夹任务：请求地址同样填 `/webhook/{任务名}`，`savepath` 填接收夹本身或它的子目录（留空即接收夹），下载完成后自动识别、按分发目标监控任务的刮削预设整理并移动过去
- 鉴权只有一套：`webhook_secret` 是全局密钥，普通监控任务和接收夹任务共用，脚本里配一次即可（接收夹只是接收后的处理流程不同）
...
- `401 Webhook 签名已被使用`：同一份签名被提交了两次。服务端防重放只认一次签名，而脚本在网络超时后会自动换通道重试，所以**看到这条多半意味着第一次请求其实已经受理**——先到「任务中心」（或接收夹）确认是否已经收到，不要连续重推；脚本 `2.7.2` 起每次重试都会重新签名，且把「同一条磁力已在处理」识别为「已提交过」，不再当成失败。
- `409 该磁力已在处理中 / 已添加过`：同一条磁力 + 同一个保存路径重复提交（与普通导入一致的查重口径），属于提示而非故障。
...
- 当前版本：`0.11.18`
- 接收夹口径与手机端推送修复：**接收夹是"分类前的中转文件夹"，也是可选的新增入口（不是强制流程）**——不用它照样按电影 / 电视剧监控目录直接推送、保存；它和普通监控任务**共用同一个 Webhook 签名密钥**（全站唯一，不需要单独配）。同时修掉手机端偶发的 `401 Webhook 签名已被使用`：脚本在网络超时后走备用通道时会**重新签名**，不再重发同一份签名（那个 401 之前的第一次请求其实已经受理，先去任务中心确认再决定是否重推）；同一条磁力已在处理（409）改提示「已提交过」，不再显示成失败。油猴脚本 `@version` 升至 `2.7.2`，README 新增「常见报错」两条。
- 识别结果可视化与准确率修复：接收夹整理 / 接收夹分发的**运行记录逐条标出「原文件名 → 识别为」**，含年份、TMDB ID、识别来源（规则/AI）、置信度与理由，错配一眼可见。「识别为什么还不准」的三个根因一并修：文件名开头方括号里的**中文片名**（如 `[朱弦玉磐2024]` 里的「朱弦玉磐」）不再被当噪声丢弃、会参与 TMDB 搜索直接命中正确条目；AI 兜底改用**文件名里的确定性年份**并拒绝与条目年份冲突的 AI 候选（避免同名异年错配）；`ai_match_min_confidence` **默认由 0（不过滤）提到 60**，旧默认 0 升级时自动升级。电影整理后的文件夹名**优先用文件名里的中文片名**（如「七勇破星阵 (1966)」），没有中文名才用 TMDB 中文名（剧集仍以 TMDB 命名为准）。
- 文件夹监控「运行记录」**一条任务一行**：接收夹整理是一条任务，它分发到监控目录的**每个条目也都会单独生成一条记录**（启动方式标为「接收夹分发」），不做分组/折叠；列表按时间排序时触发记录排在自己派生出来的任务之前（活动时间从下游上溯，同秒也保证顺序），所以“最近接收”不会夹在那 6 条任务中间。**接收夹整理不再假部分完成**：条目同步与自动补扫会挂回发起它的变更同步运行，全部结束后整条链路按真实结果定稿——6 部影视都同步成功就是「已完成」，只有真的失败 / 仍留在接收夹 / 事件未解决才「部分完成」，结论写明未完成的是哪一项；升级后首次启动会把历史上“只是等补扫、实际已全部完成”的误判记录重新结算（真实失败不动），自动补扫的运行记录标为「系统补扫」。
...
- 油猴脚本（`@version` 2.7.2）：点磁力/torrent 链接旁的「115」后，任务选择弹窗里新增与推送任务同级的「复制磁力链接」入口，torrent 会先自动转成磁力再复制，复制后弹窗保持打开；内置接收夹任务可以用同一个签名密钥直接推送（「保存路径」可留空，留空即默认落到接收夹），任务管理器说明写明「接收夹＝分类前的中转文件夹、和普通监控任务共用同一密钥」。
